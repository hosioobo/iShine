...
- Skill execution order changes in AGENTS.md
...
Target file: <exact path, e.g., .agents/skills/validate/SKILL.md>
...
### [3] High — AGENTS.md: feedback routing table
...card...
...
3. **Cross-adapter sync (scope: public only):**
   - If `.agents/skills/*` was modified → sync the corresponding `.claude/skills/*` file
   - If `AGENTS.md` engine section was modified → sync the corresponding section in `CLAUDE.md`
   - If `.claude/skills/*` or `CLAUDE.md` was the source → sync to `.agents/` / `AGENTS.md`
...
5. Append an entry to `log/changelog.md`:
   ...
   - [Category A|B] [scope] [file]: [one-line summary of change]
   - [Category A|B] [scope] [file]: [one-line summary of change]
...
`AGENTS.md` · `CLAUDE.md` · `preferences.md` · `.agents/skills/*.md` · `.claude/skills/*.md` · `templates/*` · `log/changelog.md` · new file creation (e.g., new skill SKILL.md)
...
- If a proposed change conflicts with an existing rule in AGENTS.md, flag the conflict in the change card rather than silently overriding.
