...
- **Phase 3 (reviews):** 3 sub agents in parallel. Each sees jd.md summary + resume + single persona section only. Outputs narrative review (5-section OpenAI format) saved as md per persona. Includes interview Q&A.
...
- Multi-point feedback: when user provides 4+ changes in one message, number each point in the plan and confirm full coverage before calling ExitPlanMode.
...
- scripts/ + templates/docx_spec.md — rendering, appearance, export styling. CLAUDE.md and content templates must not contain rendering rules.
